...
def separateAugmented(Aaug):
    ...
    :return: (A, b) where b is a flat list
    """
    A = [row[:-1] for row in Aaug]  # Extract A (all columns except the last)
    b = [row[-1] for row in Aaug]   # Extract b (last column as a flat list)
    return A, b
